                ...
                'service-object' => {
                    ...
                    parse =>
                        ...
                        ['cond1',
                         ...
                         { store => 'SPEC',
                           parse => 'parse_icmp_spec', params => [ 'icmp' ]}, ],
                        ['cond1',
                         { store => 'TYPE', parse => qr/icmp6/ },
                         { store => 'SPEC',
                           parse => 'parse_icmp_spec', params => [ 'icmp6' ]}, ]
                        ],
                },
                'group-object' => {
                    error => 'Nested object group not supported'
...
#              $strict_IP_version - allow only ipv4 / ipv6 addresses in ACL.
# Result     : Adjusted result hash.
sub postprocess_config {
    my ($self, $p, $strict_IP_version) = @_;
...
#        }
    ...
    # 1. Adjust any/any4 usage within ASA-Versions < 9.
    # 2. Check valid IP addresses in IPv4 / IPv6 raw file.
    $self->postprocess_ACLs($p, $strict_IP_version);
...
##############################################################################
# Purpose    : 1.)
#              Devices with Version < 9 do handle IPv6 differently.
#              Generating ACLs using 'any4' instead of 'any' (ASA Version
#              9+) Netspoc no longer supports such devices. This function
#              removes ANY46-attribute for IPv4-only devices, so approve won't
#              notice any differences between 'any' and 'any4'. As long as no
#              new 'any4' ACLs are generated by Netspoc, old devices
#              can still be approved.
#              2.)
#              ACLs in raw files are checked for IP addresses:
#              IPv6 raw file must only have IPv6 addresses,
#              IPv4 raw file must only have IPv4 addresses.
# Parameters : $config - Config hash
#              $strict - is only set when parsings raw files
#                        'ipv4': only IPv4 addresses accepted in ACL
#                        'ipv6': only IPv6 addresses accepted in ACL
# Result     : ANY46-flags are removed from ACLs.
sub postprocess_ACLs {
    my ($self, $config, $strict) = @_;
    my $ipv4_only = $self->{IPv4_only};
    $strict or $ipv4_only or return;
    my $access_lists = $config->{ACCESS_LIST};
    for my $acl_name (keys %$access_lists) {
        my $entries = $access_lists->{$acl_name}->{LIST};
        for my $entry (@$entries) {
            for my $what (qw(SRC DST)) {
                ...
                $strict or next;
                if (not $ipv4_only and $obj->{'ANY46'}) {
                    abort("Bare 'any' only allowed in raw file" .
                          " for IPv4-only device:",
                          $entry->{orig});
                }
                if (length($obj->{BASE}) == 16) {
                    $strict eq 'ipv4' and
                        abort("IPv6 address not allowed in IPv4 raw file:",
                              $entry->{orig});
                } elsif ($strict eq 'ipv6') {
                    abort("IPv4 address not allowed in IPv6 raw file:",
                          $entry->{orig});
                }
            }
        }
...
sub parse_object_group  {
    my ($self, $arg) = @_;
    check_regex('object-group', $arg) or return;
    return { GROUP_NAME => get_token($arg) };
}

sub parse_any4 {
    my ($self, $arg) = @_;
    check_regex('any4', $arg) or return;
    my $ip = my $mask = pack('N', 0);
    return ({ BASE => $ip, MASK => $mask });
}

sub parse_any6 {
    my ($self, $arg) = @_;
    check_regex('any6', $arg) or return;
    my $ip = my $mask = NetAddr::IP::Util::ipv6_aton('0:0:0:0:0:0:0:0');
    return ({ BASE => $ip, MASK => $mask });
}

# Returns    : A hash with BASE and MASK keys and values.
#              Additionally attribute ANY46 is set.
# Comments   : Attribute ANY46 is needed for Dual Stack Netspoc. While
#              'any' is not generated by Netspoc Compiler for ASA ACLs
#              anymore (generates 'any4' or 'any6' instead), 'any' could still
#              be used in raw files of IPv4 only devices or within
...
sub parse_dual_any {
    my ($self, $arg) = @_;
    check_regex('any', $arg) or return;
    my $ip = my $mask = pack('N', 0);
    return ({ BASE => $ip, MASK => $mask, ANY46 => 1 });
}
...
sub parse_address {
    ...
    return
        $self->parse_any4($arg) ||
        $self->parse_any6($arg) ||
        $self->parse_dual_any($arg) ||
        $self->parse_object_group($arg) ||
        $self->SUPER::parse_address($arg);
}
