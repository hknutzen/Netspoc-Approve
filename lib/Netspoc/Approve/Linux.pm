...
sub write_startup_routing {
    my ($self, $spoc, $file) = @_;
    local $\ = "\n";
    system("rm -f $file") == 0 or die "Can't remove $file: $?\n";
    open FILE, ">$file" or die "Cann't open $file: $!\n";
    print FILE '#!/bin/sh';
    print FILE '# Generated by NetSPoC';
...
sub write_startup_iptables {
    ...
    system("rm -f $file") == 0 or die "Can't remove $file: $?\n";
    open FILE, ">$file" or die "Cann't open $file: $!\n";
    print FILE "#!$config->{iptables_restore_cmd}";
    print FILE '# Generated by NetSPoC';
