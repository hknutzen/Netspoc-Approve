...
sub ip_netz_a_in_b ($$){
    ...
    #
    #  WARNING: DO NOT CHANGE THE RETURN VALUES! THEY ARE USED IN  static_global_local_match_a_b()
    #
    my $am = $_[0]->{MASK};
    my $bm = $_[1]->{MASK};
    my $mm = $am & $bm;
...
sub acl_line_a_eq_b ($$){
    ...
    my $bsrc = $$b{PROTO}->{SRC};
    my $bdst = $$b{PROTO}->{DST};
    unless($$a{MODE} eq $$b{MODE} && $$a{PROTO}->{TYPE} eq $$b{PROTO}->{TYPE} &&
	   $asrc->{BASE} == $bsrc->{BASE} && $asrc->{MASK} == $bsrc->{MASK} &&
	   $adst->{BASE} == $bdst->{BASE} && $adst->{MASK} == $bdst->{MASK}){
	return 0;
    }
    ...
    elsif($$a{PROTO}->{TYPE} eq 'tcp' or $$a{PROTO}->{TYPE} eq 'udp' or
	  $$a{PROTO}->{TYPE} eq 6 or $$a{PROTO}->{TYPE} eq 17){
	my $ass  = $asrc->{SRV};
	my $ads  = $adst->{SRV};
	my $bss  = $bsrc->{SRV};
	my $bds  = $bdst->{SRV};
	...
	if($ass->{SPEC}){
	    ...
	    ($ass->{PORT_L} == $bss->{PORT_L} and
	     $ass->{PORT_H} == $bss->{PORT_H}) or return 0;
	}
	if($ads->{SPEC}){ 
	    ($ads->{SPEC} eq $bds->{SPEC}) or return 0;
	    ($ads->{PORT_L} == $bds->{PORT_L} and
	     $ads->{PORT_H} == $bds->{PORT_H}) or return 0;
	}
	# Ports are equal
...
sub route_line_a_eq_b( $$$ ){
    ...
    ($a->{BASE} eq $b->{BASE} && 
     $a->{MASK} eq $b->{MASK}) or return 0;
    if(defined($a->{NEXTHOP}) || defined($b->{NEXTHOP})){
	((defined($a->{NEXTHOP}) && defined($b->{NEXTHOP}) &&
	  $a->{NEXTHOP} eq $b->{NEXTHOP})) or return 0;
    }
    if(defined($a->{NIF}) || defined($b->{NIF})){
	((defined($a->{NIF}) && defined($b->{NIF}) &&
	  $a->{NIF} eq $b->{NIF})) or return 0;
    }
    if(defined($a->{MISC}) || defined($b->{MISC})){
	((defined($a->{MISC}) && defined($b->{MISC}) &&
	  $a->{MISC} eq $b->{MISC})) or return 0;
    }
    if(defined($a->{TRACK_NUMBER}) || defined($b->{TRACK_NUMBER})){
	((defined($a->{TRACK_NUMBER}) && defined($b->{TRACK_NUMBER}) &&
	  $a->{TRACK_NUMBER} eq $b->{TRACK_NUMBER})) or return 0;
    }
    return 1;
}
...
sub route_line_a_supersedes_b( $$$ ){
	...
	if(defined($a->{NEXTHOP}) || defined($b->{NEXTHOP})){
	    ((defined($a->{NEXTHOP}) && defined($b->{NEXTHOP}) &&
	      $a->{NEXTHOP} eq $b->{NEXTHOP})) and return 0;
	}
	if(defined($a->{NIF}) || defined($b->{NIF})){
	    ((defined($a->{NIF}) && defined($b->{NIF}) &&
	      $a->{NIF} eq $b->{NIF})) and return 0;
	}
	return 1;
    }
...
sub hash_acl( $$ ){
	...
	for my $asm (keys %$msk){
		...
		push @{$H->{$prot}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$bsm&$bsb&$asm}->{$bdm&$bdb&$adm}},[$ace,$i];
	    }
	}
    }   
    return $H
...
sub get_hash_matches( $$$ ){
	...
	for my $bdm (keys %{$msk->{$bsm}}){
	    if($proto eq 'ip'){
		for my $p (keys %$H){
		    if(defined $H->{$p}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm}){
			$found{$H->{$p}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm}} = 
			    $H->{$p}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm};
		    } 
		}
	    }
	    else{
		if(defined $H->{$proto}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm}){
		    $found{$H->{$proto}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm}} = 
			$H->{$proto}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm};
		} 
		if(defined $H->{'ip'}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm}){
		    $found{$H->{'ip'}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm}} = 
			$H->{'ip'}->{$bsm}->{$bdm}->{$asm}->{$adm}->{$asm&$asb&$bsm}->{$adm&$adb&$bdm};
		} 
	    }
	}
    }
...
sub show( $$$ ){
	...
	for my $a (@$found){
	    print "  ($c) ",$self->acl_line_to_string($a->[0])," -> $a->[1]\n";
	    ++$c;
	}
	print "\n";
    }  
...
sub acl_array_compare_a_in_b ( $$$$$ ){
  ...
  OUTER: for my $s (@$ac){
	  ...
	  for my $deny (@ad){
		  ...
		  ($verbose) and do {
		      print "**** VERBOSE (fill currentdenylist) **** ",$lines," : ";
		      print $self->acl_line_to_string($s);
      ...
      my @deny_int =();
      my $deny_match = "NO";
      my @found =  sort {$a->[1] <=> $b->[1]} @{get_hash_matches($s,$bmsk,$bhsh)};
      my $p;
	...
	if ($self->acl_line_a_in_b ($s,$p) == 1){
		...
		#check if found denys subset of @cdlist
		  ...
		  for my $cd (@currentdenylist){
			  ...
			  ($verbose) and do {
			      ...
			      print " partial ";
			      print $$deny[0]," : ",$self->acl_line_to_string($$deny[1]);
			      print " has full match at left side: (";
			      print $$cd[0],"): ",$self->acl_line_to_string($$cd[1]);
			      print "\n";
			  };
			  next CHECK;
		      }
	    ...
	    if($lm and !$silent){
		print "**** LOG MISMATCH **** ($lines): ",$self->acl_line_to_string($s);
		print " in ($inner): ",$self->acl_line_to_string($p),"\n";
	    }
	    elsif($showmatches){
		print "**** SHOW MATCHES **** ($lines): ",$self->acl_line_to_string($s);
		print " in ($inner): ",$self->acl_line_to_string($p),"\n";
	    }
	    next OUTER;
	}
		...
		if($verbose){
		    print "($lines): ",$self->acl_line_to_string($s);
		    print " INTERSECTION ($inner): ",$self->acl_line_to_string($p);
		    print "\n";
		}
	    }
	}
