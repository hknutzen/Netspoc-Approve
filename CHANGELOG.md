...
- Changes to PAN-OS devices are no longer committed in one run, but one
  by one for each VSYS. This works around a limitation that the
  nummber of uncommitted changes must not exceed 250.

## [2025-12-16-1656]

### Added

- Support for checkpoint firewalls has been added.
  Rules, groups, networks, hosts, protocols and static routes are changed
  to values generated by Netspoc.

### Fixed

- IPv6 addresses in routes are compared case insensitively for Cisco devices.
