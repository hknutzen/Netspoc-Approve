...
func (s *State) getDeviceRoutes() []string {
	...
	lines = slices.DeleteFunc(lines, func(l string) bool {
		if strings.Contains(l, " scope link") {
			return true
		}
		if m, _ := regexp.MatchString(l, ` proto (?:kernel|boot|[0-9]+)`); m {
			return true
		}
		return false
	})
	rx := regexp.MustCompile(`( via \S+) dev \S+`)
	for i, l := range lines {
		// Ignore attribute 'dev', if 'via' is provided.
		if s := rx.FindStringSubmatchIndex(l); s != nil {
			l = l[:s[0]] + l[s[2]:s[3]] + l[s[1]:]
		}
		// Change 'default' to '0.0.0.0/0'
		if rest, found := strings.CutPrefix(l, "default"); found {
			l = "0.0.0.0/0" + rest
		}
...
func (s *State) writeStartupRouting(routes []string, dst string) {
	file := createTemp("routes")
	defer os.Remove(file.Name())
	lines := append([]string{"#!/bin/sh", "# Generated by NetSPoC"}, routes...)
